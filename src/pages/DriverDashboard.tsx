...
interface CurrentJob {
  ...
  status: 'pending_acceptance' | 'assigned' | 'arrived' | 'transporting' | 'delivered' | 'completed';
  assignedTime: string;
...
const STATUS_STEPS = ['pending_acceptance', 'assigned', 'arrived', 'transporting', 'delivered', 'completed'];
const STEP_LABELS = ['Pending', 'Assigned', 'Arrived', 'Transporting', 'Delivered', 'Completed'];
...
const DriverDashboard: React.FC = () => {
  ...
  const getStatusStep = (status: string): number => {
    ...
    if (status === 'transporting') return 3;
  ...
  const handleAcceptIncident = async (incident: any) => {
    try {
      ...
      if (result.success) {
        ...
        SocketService.emitDriverResponse(targetId, 'accept');
  ...
  const handleRejectIncident = async (incident: any, reason?: string) => {
    try {
      ...
      if (result.success) {
        ...
        SocketService.emitDriverResponse(targetId, 'reject');
  ...
  const handleHospitalSelection = (): void => {
    if (selectedHospitalObj) {
      ...
      const hospital = nearbyHospitals.find(h => h.name === selectedHospital);
      if (hospital) {
  ...
  const getStatusBannerConfig = (status: string) => {
    switch (status) {
      ...
      case 'assigned':
        return { color: '#FF9800', icon: <CarIcon />, message: 'Tap "Start Ride" to navigate to the incident scene.' };
      case 'arrived':
        return { color: '#1976D2', icon: <HospitalIcon />, message: 'Tap "Enable Hospital Route" to find the nearest hospital.' };
      case 'transporting':
        return { color: '#9C27B0', icon: <MedicalServicesIcon />, message: `Patient on board. Navigate to ${selectedHospitalObj?.name || selectedHospital || 'hospital'}.` };
      case 'delivered':
    ...
    <Container maxWidth={false} sx={{ py: 4, px: { xs: 2, sm: 3, md: 4 }, minHeight: '100vh', bgcolor: '#F5F5F5' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Box
              sx={{
                ...
                height: 12,
                borderRadius: '50%',
                bgcolor: driverStats.activeAssignments === 0 ? '#4CAF50' : '#FF9800',
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (currentJob?.incident?.location?.address) {
                        window.open(`https://maps.google.com/?q=${encodeURIComponent(currentJob.incident.location.address)}`, '_blank');
                      } else {
      ...
      {currentTabIndex === 2 ? (
        <Grid container spacing={3}>
              ...
              <Card elevation={3} sx={{ borderRadius: 3, overflow: 'hidden' }}>
                <CardContent sx={{ p: 3 }}>
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                    <Typography variant="h5" fontWeight={600}>Current Assignment</Typography>
                    <Box display="flex" gap={1}>
                      <Chip 
                        ...
                        size="small"
                      />
                      {currentJob.status === 'pending_acceptance' && countdown !== null && (
                        <Chip
                          icon={<AccessTimeIcon />}
                          label={formatCountdown(countdown)}
                          sx={{ bgcolor: getCountdownColor(countdown), color: 'white', fontWeight: 'bold' }}
                        />
                      )}
                    </Box>
                  </Box>
                ...
                <CardActions sx={{ p: 3, pt: 0, justifyContent: 'center' }}>
                  {currentJob.status === 'pending_acceptance' && (
                    <Box sx={{ width: '100%' }}>
                      <Grid container spacing={2}>
                        <Grid item xs={6}>
                          <Button 
                            ...
                            color="success"
                            fullWidth
                            onClick={() => handleAcceptIncident(currentJob.incident)} 
                        ...
                        <Grid item xs={6}>
                          <Button 
                            ...
                            color="error"
                            fullWidth
                            onClick={() => handleRejectIncident(currentJob.incident, 'Driver rejected')} 
                      ...
                      {countdown !== null && (
                        <Box sx={{ mt: 2 }}>
                          <LinearProgress 
                            ...
                            sx={{ height: 8, borderRadius: 4, '& .MuiLinearProgress-bar': { bgcolor: getCountdownColor(countdown) } }}
                          />
                        </Box>
                      )}
                    </Box>
              ...
              <CardContent>
                ...
                <List dense>
                  {nearbyHospitals.slice(0, 3).map((hospital) => (
                    <ListItem key={hospital.id} sx={{ px: 0 }}>
                      <ListItemIcon><HospitalIcon color="primary" /></ListItemIcon>
                      <ListItemText 
                        primary={hospital.name} 
                        secondary={`${hospital.type} • ${hospital.distance} • ${hospital.etaMinutes} min`}
                      />
                      {selectedHospitalObj?.id === hospital.id && <CheckIcon color="success" fontSize="small" />}
                    </ListItem>
                  ))}
                </List>
                <Button 
                ...
                >
                  {loadingHospitals ? <CircularProgress size={20} /> : 'View All Hospitals'}
                </Button>
              </CardContent>
            </Card>
            ...
            <Card sx={{ borderRadius: 3 }}>
              <CardContent>
                ...
                <Box display="flex" alignItems="center" gap={1} mb={1}>
                  ...
                  <Typography variant="body2">{driverAddress}</Typography>
                </Box>
                {currentLocation && (
                  <Typography variant="caption" color="text.secondary">
                ...
                <Button size="small" startIcon={<RefreshIcon />} onClick={getCurrentLocation} sx={{ mt: 1 }}>
                  Refresh Location
                </Button>
              </CardContent>
            </Card>
          ...
          {/* All Active Incidents List */}
          <Grid item xs={12}>
            <Card sx={{ borderRadius: 3 }}>
              <CardContent>
                <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                  <Typography variant="h6">Available Incidents</Typography>
                  <Chip label={`${assignedIncidents.length + pendingIncidents.length} total`} size="small" color="primary" variant="outlined" />
                </Box>
                
                {(assignedIncidents.length + pendingIncidents.length) === 0 ? (
                        ...
                        <React.Fragment key={incident.id}>
                          <ListItem 
                            ...
                            sx={{
                              borderRadius: 2,
                              mb: 1,
                              bgcolor: isSelected ? '#f8d7d7' : '#fff4e5',
                              border: '1px solid #ff9800',
                              '&:hover': { bgcolor: '#f8d7d7' },
                            }}
                          >
                            <ListItemIcon>
                            ...
                            <ListItemText
                              primary={
                                <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
                                  ...
                                  <Chip label="Pending Acceptance" size="small" color="warning" sx={{ height: 20, fontSize: '0.7rem' }} />
                                  <Chip 
                                    icon={<AccessTimeIcon />} 
                                    label={formatCountdown(timeRemaining)} 
                                    size="small"
                                    sx={{ bgcolor: getCountdownColor(timeRemaining), color: 'white', height: 20, fontSize: '0.7rem' }}
                                  />
                                </Box>
                              }
                              secondary={
                                <Typography variant="caption" color="text.secondary">
                                  {new Date(incident.createdAt).toLocaleTimeString()} • Need response
                                </Typography>
                              }
                            />
                            {isSelected ? <CheckIcon color="primary" /> : <ArrowForwardIcon color="action" />}
                          </ListItem>
                        ...
                        <React.Fragment key={incident.id}>
                          <ListItem 
                            ...
                            sx={{
                              borderRadius: 2,
                              mb: 1,
                              bgcolor: isSelected ? '#f8d7d7' : 'transparent',
                              '&:hover': { bgcolor: '#f8d7d7' },
                            }}
                          >
                            <ListItemIcon>
        ...
        <Grid container spacing={3}>
          {pendingIncidents.length === 0 ? (
            ...
            pendingIncidents.map(incident => {
              ...
              return (
                <Grid item xs={12} md={6} key={incident.id}>
                  <Card sx={{ borderRadius: 3, border: '2px solid #FF9800', bgcolor: '#FFFDE7' }}>
                    <CardContent>
                      <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={1.5}>
                        <Box>
                          <Typography variant="subtitle1" fontWeight={700}>
