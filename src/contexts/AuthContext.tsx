...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const silentAdminLogin = async (): Promise<boolean> => {
  try {
    // Use stored admin credentials or get from env
    const response = await axios.post('/api/auth/login', {
      email: 'admin@irs.com', // Your admin email
      password: '123456'       // Your admin password
    });
  ...
  try {
    ...
    if (error.response?.status === 403) {
      // Try silent login once
      const loggedIn = await silentAdminLogin();
      if (loggedIn) {
        // Retry the request
        const retryResponse = await axios.get("/api/admin/incidents", { params });
